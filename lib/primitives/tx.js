/*!
 * tx.js - transaction object for bcash
 ...
 * @property {Number} flag - Flag field.
 * Always non-zero (2 if not present).
 ...
 * (calculated against bytes-per-sigops).
 * @param {Number} sigops - Sigops count.
 * @returns {Number} size
 */

TX.prototype.getSigopsSize = function getSigopsSize(sigops) {
  const bytes = policy.BYTES_PER_SIGOP;
  const size = Math.max(this.getBaseSize(), sigops * bytes);
  return size;
};
...
TX.prototype.signatureHash = function signatureHash(index, prev, value, type, flags) {
...
TX.prototype.signatureHashV2 = function signatureHashV2(index, prev, value, type) {
  const input = this.inputs[index];
  let prevouts = encoding.ZERO_HASH;
  let sequences = encoding.ZERO_HASH;
  let outputs = encoding.ZERO_HASH;

  if ((type & hashType.SIGHASH_FORKID)
...
TX.prototype.checksig = function checksig(index, prev, value, sig, key, flags) {
  if (sig.length === 0)
    return false;
...
TX.prototype.signature = function signature(index, prev, value, key, type, flags) {
  if (type == null)
    type = hashType.SIGHASH_FORKID;
...
TX.prototype.getSigopsCount = function getSigopsCount(view, flags) {
 if (flags == null)
    flags = Script.flags.STANDARD_VERIFY_FLAGS;

  let sigops = this.getLegacySigops();

  if (this.isCoinbase())
    return sigops;

  if (flags & Script.flags.VERIFY_P2SH)
    sigops += this.getScripthashSigops(view)
...
TX.prototype.checkSanity = function checkSanity() {
  ...
  if (this.getBaseSize() > consensus.ONE_MEGABYTE)
  ...
  if (this.getLegacySigops() > policy.MAX_TX_SIGOPS)
    return [false, 'bad-txns-sigops', 100];
...
TX.prototype.checkStandard = function checkStandard() {
  ...
  if (this.getBaseSize() >= policy.MAX_TX_SIZE)
