...
const cashaddr = require('../utils/cashaddr');
...
/**
 * Compile the address object to a CashAddress.
 * @param {{NetworkType|Network?}} network
 * @returns {CashAddress}
 * @throws Error on bad hash/prefix
 */
...
Address.prototype.toCashAddr = function toCashAddr(network) {
  ...
  const hrp = network.addressPrefix.cashaddr;

  return cashaddr.encode(hrp, version, data);
};
...
/**
 * Inject properties from CashAddresses.
 * @private
 * @param {CashAddr} data
 * @param {Network?} network
 * @throws Parse error
...
Address.prototype.fromCashAddr = function fromCashAddr(data, network) {
  assert(typeof data === 'string');
  const addr = cashaddr.decode(data)

  network = Network.fromCashAddr(addr.hrp, network);

  return this.fromHash(addr.hash, type, addr.version, network);
};

/**
 * Create an address object from a cashaddress.
 * @param {CashAddr} data
 * @param {Network?} network
 * @returns {Address}
 * @throws Parse error.
...
Address.fromCashAddr = function fromCashAddr(data, network) {
  return new Address().fromCashAddr(data, network);
};
...
Address.prototype.fromHash = function fromHash(hash, type, version, network) {
  ...
  assert(type >= Address.types.PUBKEYHASH,
