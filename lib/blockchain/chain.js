...
class Chain extends AsyncEmitter {
  ...
  async getDeployments(time, prev) {
    ...
    // Difficulty Adjustment Algorithm activation.
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.isDAAEnabled() && state.isDAAEnabled())
      this.logger.warning('Difficulty Adjustment Algorithm Activation.');
...
async getNextWorkRequired(time, prev) {
  const pow = this.network.pow;
  const height = prev.height + 1;
 ...
 */
  ...
  computeTarget(prev, first) {
    ...
    // expect to be produced in the targetted time
    // between blocks.
    ...
    work = work.divn(actualTimespan);

    /*
     * We need to compute T = (2^256 / work) - 1
     * but 2^256 doesn't fit in 256 bits.
     * By Expression 1 as a valid Chainwork entry,
     * we'll be able to get (2^256 - work) / work,
     * therefore we'll be able to compute 2^256 - work as
     * the complement of the Chainwork.
     */
  ...
  getSuitableBlock(prev) {
    assert(prev.height >= 3);

    const height = prev.height + 1;
    const block2 = prev;
    const block1 = this.getEntryByHeight(height - 1);
    const block0 = this.getEntryByHeight(height - 2);
  ...
  /**
   * Get Chain constants activation for Hard Fork
   * @param {ChainEntry} prev - Previous chain entry.
   * @returns {Promise}
   */

  async isUAHFEnabled(prev) {
    const state = new DeploymentState();
    const height = prev.height + 1;
 ...
 /**
  * When Enabled, we start rejecting transactions
  * which use HIGH-S Flags in their signature.
  * We secure that each Signature using these sigs
  * will fail if used.
  * @param {ChainEntry} prev - Previous chain entry.
  * @returns {Promise}
  */

  async isDAAEnabled(prev) {
    const height = prev.height + 1;

    if (height >= this.network.block.daaHeight) {
      this.state.flags |= Script.flags.VERIFY_LOW_S;
      this.state.flags |= Script.flags.VERIFY_NULLFAILL;
    }
    return this.state;
  }
...
class DeploymentState {
  ...
  /**
   * Test whether the UAHF Sighashing is active.
   * @returns {Boolean}
   */
  ...
  /**
   * Test whether the new LOW_S is enabled for blocks.
   * @returns {Boolean}
   */
