...
Chain.prototype.verify = async function verify(block, prev, flags) {
  ...
  const bits = await this.getNextWorkRequired(block.time, prev);
...
/**
 * Check all deployments on a chain, ranging from p2sh to UAHF / DAA.
 * @param {Number} time
 * @param {ChainEntry} prev
 * @returns {Promise} - Returns {@link DeploymentState}.
 */
...
Chain.prototype.getDeployments = async function getDeployments(time, prev) {
  ...
  // UAHF enforced, we start accepting replay protected txns.
  ...
  // Difficulty Adjust Algorithm activation.
...
Chain.prototype.setDeploymentState = function setDeploymentState(state) {
  ...
  if (!this.state.isDAAEnabled() && state.isDAAEnabled())
    this.logger.warning('Difficulty Adjustment Algorithm activated.');
...
Chain.prototype.verifyInputs = async function verifyInputs(block, prev, state) {
  ...
  const maxBlockSigops = consensus.getMaxBlockSigopsCount(block.getBaseSize());
  ...
  for (let i = 0; i < block.txs.length; i++) {
    ...
    // When P2SH enabled in flags excluding Coinbase
...
/**
 * Calculate the new target based on chain tip.
 * @returns {Promise} - returns Number
 * (target is in compact/mantissa form).
 */

Chain.prototype.getDAATarget = async function getDAATarget() {
  return await this.daaRetarget(this.network.now(), this.tip);
};
...
Chain.prototype.getTarget = async function getTarget(time, prev) {
  ...
  const mtp6 = await this.getMedianTime(prev6);
...
Chain.prototype.daaRetarget = async function daaRetarget(time, prev) {
  const pow = this.network.pow;
  ...
  const height = prev.height + 1;
  assert(height >= this.network.pow.retargetInterval);

  const last = await this.getSuitableBlock(prev);
  assert(last);

  // First suitable block of the difficulty interval.
  const first = await this.getAncestor(prev, (height - 144));
  assert(first);

  // Compute the target based on time and work done during the interval.
  const nextTarget = await this.computeTarget(first, last);

  if (nextTarget.cmp(pow.limit) > 0)
    return pow.bits;

  return consensus.toCompact(nextTarget);
};
...
Chain.prototype.getNextWorkRequired = async function getNextWorkRequired(time, prev) {
  const pow = this.network.pow;
  const height = prev.height + 1;
  ...
  if (prev.height >= this.network.block.daaHeight)
    return this.daaRetarget(time, prev);

  return this.getTarget(time, prev);
}
 ...
 * @param {ChainEntry} prev - Previous entry.
 * @param {ChainEntry} first - Chain entry from 2 weeks prior.
 * @returns {Number} work - work in compact/mantissa form.
 */

Chain.prototype.computeTarget = function computeTarget(prev, first) {
  const pow = this.network.pow;
  const targetSpacing = pow.targetSpacing;
  let work;

  if (prev.chainwork.gt(first.chainwork) > 0) {
    work = prev.chainwork.sub(first.chainwork);
  } else {
    work = first.chainwork.sub(prev.chainwork);
  }

  // From the total chainwork done along with
  // the time it took to produce the chainwork,
  // we'll be able to deduce how much work we
  // expect to be produced in the targeted time
  // between blocks.

  let actualTimespan = prev.time - first.time;

  if (actualTimespan > 288 * targetSpacing) {
    actualTimespan = 288 * targetSpacing;
  }
  else if (actualTimespan < 72 * targetSpacing) {
    actualTimespan = 72 * targetSpacing;
  }

  work = work.imuln(targetSpacing);
  work = work.idivn(actualTimespan);

  /*
   * to Compute 2^256 - work as the complement of the Chainwork.
   * We need to compute T = (2^256 / work) - 1 but 2^256 doesn't fit
   * in 256 bits. By expressing 1 as work,
   * we'll get (2^256 - work) / work, therefore we'll be able
   */

  return work.div(work);
};

/*
 * We select the median of the nearest blocks.
 * @param {Block[]}
 * @returns {Promise}
 */

Chain.prototype.getSuitableBlock = async function getSuitableBlock(prev) {
  assert(prev.height >= 3);

  const height = prev.height + 1;
  const block2 = prev;
  const block1 = await this.getEntryByHeight(prev.height - 1);
  const block0 = await this.getEntryByHeight(prev.height - 2);

  // Sorting network.
  if (block0.time > block2.time) {
    return swap(block0, block2);
  }

  if (block0.time > block1.time) {
   return swap(block0, block1);
  }

  if (block1.time > block2.time) {
    return swap(block1, block2);
  }
 ...
 * @example
 * await chain.isActive(tip, deployments.csb);
...
/**
 * Get Chain Constant Activation for Hard Fork
 * @param {ChainEntry} prev - Previous chain entry.
 * @returns {Promise}
 */

Chain.prototype.isUAHFEnabled = async function isUAHFEnabled(prev) {
  const height = prev.height + 1;

  if (height >= this.network.block.UAHFHeight) {
    this.state.flags |= Script.flags.VERIFY_STRICTENC;
    this.state.flags |= Script.flags.VERIFY_SIGHASH_FORKID;
  }
...
/**
 * When Enabled, we start rejecting transactions which
 * use HIGH-S Flags in their signature. We Secure that
 * each Signature using these signatures will fail if used.
 * @param {ChainEntry} prev - Previous chain entry.
 * @returns {Promise}
 */

Chain.prototype.isDAAEnabled = async function isDAAEnabled(prev) {
  const height = prev.height + 1;

  if (height >= this.network.block.daaHeight) {
    this.state.flags |= Script.flags.VERIFY_LOW_S;
    this.state.flags |= Script.flags.VERIFY_NULLFAIL;
  }
 ...
 * @example
 * await chain.getState(tip, deployments.csv);
 * @see https://github.com/bitcoin/bips/blob/master/bip-0009.mediawiki
 * @param {ChainEntry} prev - Previous chain entry.
 * @param {String} id - Deployment id.
 * @returns {Promise} - Returns Number.
...
/**
 * Test whether the UAHF is active
 * @returns {Boolean}
 */
...
/**
 * Test whether the new POW is Activated.
 * @returns {Boolean}
 */

DeploymentState.prototype.isDAAEnabled = function isDAAEnabled() {
  return (this.flags & Script.flags.VERIFY_LOW_S) !== 0;
};
