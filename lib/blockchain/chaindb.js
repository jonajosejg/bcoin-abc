...
ChainDB.prototype.verifyFlags = async function verifyFlags(state) {
  ...
  if (options.ENABLE_SIGHASH_FORKID && !flags.ENABLE_SIGHASH_FORKID) {
    if (!options.forceFlags)
      throw new Error('Cannot retroactivaly enable SIGHASH_FORKID flag.');
    needsSave = true;
  }
...
function ChainFlags(options) {
  ...
  this.spv = false;
  this.ENABLE_SIGHASH_FORKID = false;
  this.prune = false;
  this.indexTX = false;
...
ChainFlags.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.ENABLE_SIGHASH_FORKID != null) {
    assert(typeof options.ENABLE_SIGHASH_FORKID === 'boolean');
    this.ENABLE_SIGHASH_FORKID = options.ENABLE_SIGHASH_FORKID;
  }
...
ChainFlags.prototype.fromRaw = function fromRaw(data) {
  ...
  this.ENABLE_SIGHASH_FORKID = (flags & 65536) !== 0;
