...
Mempool.prototype.verify = async function verify(entry, view) {
  ...
  // Since the coinbase transaction itself can
  // contain sigops. MAX_TX_SIGOPS_COUNT is less
  // than MAX_SIGOPS_BYTES; we still consider
  // this as invalid rather than merely non-standard-transaction.
...
Mempool.prototype.storeOrphan = function storeOrphan(tx, missing, id) {
  if (tx.getBaseSize() > policy.MAX_TX_SIZE) {
