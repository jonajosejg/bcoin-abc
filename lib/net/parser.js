/*!
 * parser.js - packet parser for bcoin / bcoin-abc
...
Parser.prototype.altfeed = function altfeed(data) {
  this.total += data.length;
  this.pending.push(data);
...
Parser.prototype.alternateParse = function alternateParse(data) {
  assert(data.length <= common.MAX_MESSAGE);

  if (!this.header) {
    this.header = this.parseAlternateHeader(data);
    return;
  }
...
/**
 * Parse buffered packet BCH-Header.
 * @param {Buffer} data - Header.
 * @returns {Header}
 */

Parser.prototype.parseAlternateHeader = function parseAlternateHeader(data) {
  const netmagic = data.readUInt32LE(0, true);

  if (netmagic !== this.network.netmagic) {
    this.error('Invalid magic value: %s.', util.hex32(netmagic));
    return null;
  }
