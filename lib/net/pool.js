...
Pool.prototype.sendBlock = async function sendBlock(peer, item) {
  ...
  if (block) {
    peer.send(new packets.BlockPacket(block));
    return true;
  }
...
/** Whether UAHF is enabled
 * @private
 * @returns {boolean}
 */

PoolOptions.prototype.UAHF = function UAHF() {
  return this.chain.state.hasUAHF();
};
/**
