/*!
 * peer.js - peer object for bcoin-ahc
...
Peer.prototype.frameAltPacket = function frameAltPacket(cmd, payload, checksum) {
  if (this.bip151 && this.bip151.handshake)
    return this.bip151.packet(cmd, payload);
  return this.framer.Packet(cmd, payload, checksum);
};
...
Peer.prototype.feedAltParser = function feedAltParser(data) {
  if (this.bip151 && this.bip151.handshake)
    return this.bip151.feed(data);
  return this.parser.altfeed(data);
}
...
Peer.prototype.altbind = function altbind(socket) {
  assert(!this.socket);
...
Peer.prototype.altaccept = function altaccept(socket) {
  assert(!this.socket);

  this.address = NetAddres.fromSocket(socket, this.network);
  this.address.services = 0;
  this.time = util.ms();
  this.outbound = false;
  this.connected = true;
...
Peer.prototype.altconnect = function altconnect(addr) {
  assert(!this.socket);
...
/**
 * Calculate peer tx inv type (non-witness).
 * @returns {Number}
 */
...
Peer.prototype.hasCashService = function hasCashService(services) {
  return (this.services & common.services.BITCOIN_CASH) !== 0;
};
...
Peer.prototype.hasCompactSupport = function hasCompactSupport() {
  ...
  return this.version >= common.COMPACT_VERSION;
};
...
/**
 * Test whether the new BCH-Bitset if active.
 * @private
 * @returns {Boolean}
 */

PeerOptions.hasCashService = function hasCashService() {
  return true;
};
