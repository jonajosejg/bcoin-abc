...
class Peer extends EventEmitter {
  ...
  frameAltPacket(cmd, payload, checksum) {
    if (this.bip151 && this.bip151.handshake)
      return this.bip151.packet(cmd, payload);
    return this.framer.altPacket(cmd, payload, checksum);
  }
  ...
  /**
   * Test whether the required BCH Bit set service is available.
   * For (Peer-Preferrences)
   * @returns {Boolean}
   */

  hasCashService() {
    return (this.services & services.BITCOIN_CASH) !== 0;
  }
  ...
  hasCompactSupport() {
    ...
    if (!(this.services & services.BCASH))
      return false;
...
class PeerOptions {
  ...
  constructor(options) {
    ...
    this.hasCashService = PeerOptions.hasCashService;
  ...
  fromOptions(options) {
    ...
    if (options.hasCashService != null) {
      assert(typeof options.hasCashService === 'function');
      this.hasCashService = options.hasCashService;
    }
  ...
  /**
   * Whether bcash is enabled.
   * @private
   * @returns {Boolean}
   */

  static hasCashService(socket) {
    return peer.altaccept(socket);
  }
