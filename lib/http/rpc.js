/*!
 * rpc.js - bitcoind-compatible json rpc for bcoin-abc.
...
RPC.prototype._createTemplate = async function _createTemplate(maxVersion, coinbase, rules) {
  ...
  const json = {
    ...
    sigoplimit: consensus.getMaxBlockSigopsCount(consensus.MAX_GENERATED_BLOCK_SIZE),
  ...
  if (prev) {
    ...
    if (block.getCommitmentHash()) {
      ...
      if (!tx.hasWitness()) {
        ...
        input.script.push(encoding.ZERO_HASH);
        input.script.compile();
