/*!
 * script.js - script interpreter for bcoin / bcash
 * Copyright (c) 2014-2015, Fedor Indutny (MIT License)
 * Copyright (c) 2014-2017, Christopher Jeffrey (MIT License)
 * Copyright (c)      2017, Jonathan Gonzalez (MIT License)
...
Script.prototype.execute = function execute(stack, flags, tx, index, value) {
  if (flags == null)
  ...
  for (let ip = 0; ip < this.code.length; ip++) {
    ...
    switch (op.value) {
      ...
      case opcodes.OP_XOR: {
        ...
        const v1 = stack.getNum(-2);
        const v2 = stack.getNum(-1);
        const cmp = !v1.eq(v2);

        if (!ScriptNum.fromBool(cmp))
          throw new ScriptError('INVALID_OPERAND_SIZE');

        // In order to avoid allocating, we only modify v1 in place.
        switch (op.value) {
           case opcodes.OP_AND:
      ...
      case opcodes.OP_CHECKMULTISIGVERIFY: {
        ...
        // Conversion operations. TODO: Refactor
      case opcodes.OP_BIN2NUM: {
        ...
        if (!ScriptNum.isMinimal(n)) {
          throw new ScriptError('INVALID_NUMBER_RANGE');
        }
        break;
      }
      default: {
...
/**
 * Inject properties from a block commitment.
 * @private
 * @param {Buffer} hash
...
Script.verify = function verify(input, output, tx, index, value, flags) {
  if (flags == null)
    flags = Script.flags.STANDARD_VERIFY_FLAGS;
  ...
  // SIGHASH_FORKID enabled, we also ensure Strict Encoding.
    if (flags & Script.flags.VERIFY_SIGHASH_FORKID)
...
function validateSignature(sig, flags) {
  ...
  if (flags & Script.flags.VERIFY_STRICTENC) {
    ...
    const usesFORKID = sig[sig.length - 1] & Script.hashType.SIGHASH_FORKID;
    const FORKIDEnabled = flags & Script.flags.VERIFY_SIGHASH_FORKID;
