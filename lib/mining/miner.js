/*!
 * miner.js - block generator for bcoin-abc
...
Miner.prototype.assemble = function assemble(attempt) {
  ...
  assert(attempt.size <= consensus.MAX_GENERATED_BLOCK_SIZE,
...
function MinerOptions(options) {
  ...
  this.version = -1;
  this.addresses = [];
  this.coinbaseFlags = Buffer.from('mined by bcoin-abc', 'ascii');
  this.preverify = false;

  this.minSize = 0;
  this.maxSize = consensus.MAX_BLOCK_SIZE;
  this.prioritySize = 0;
...
MinerOptions.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.prioritySize != null) {
    assert(util.isU32(options.prioritySize));
  ...
  if (options.reservedSize != null) {
    assert(util.isU32(options.reservedSize));
